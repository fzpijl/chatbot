...
const ApiKeyModal: React.FC<ApiKeyModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 transition-opacity duration-300">
      <div className="bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-md m-4 transform transition-all duration-300 scale-95 opacity-0 animate-fade-in-scale">
        <h2 className="text-xl font-bold text-white mb-4">Manage API Keys</h2>
        <p className="text-gray-400 text-sm mb-6">Your keys are stored securely in your browser's local storage and are never sent to any servers other than the respective AI providers.</p>
        ...
        <div className="mt-6 flex justify-end space-x-3">
          ...
          <button onClick={handleSave} className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-400 transition-colors">Save Keys</button>
        </div>
      </div>
      <style>{`
